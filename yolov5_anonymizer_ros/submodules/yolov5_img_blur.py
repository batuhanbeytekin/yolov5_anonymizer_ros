...
if __name__ == '__main__':
    ...
    a.add_argument("--imagePath", default="./datasets/test_images/*", help="path to input images")
    a.add_argument("--modelPath", default="./weights/best.onnx", help="path to model")
    args = a.parse_args()

    count = 0

    images = glob(args.imagePath)
    for img_path in tqdm(images):
        ...
        model_file = args.modelPath # your model path, model_path

        img_size = [640,640]

        om = yolov5onnx(model_file, nc=1, model_hw=img_size, mem_limit=800)
        
        om.pre_processing(img)
        om.inference()
        om.post_processing(img_path, img)
        count =+ count + 1
        cv2.imwrite(filename + "blur.jpg", img) 
